...
class EmotionDataView(APIView):
    def get(self, request):
        ...
        if time_threshold:
            emotion_counts_list = Employee_Emotion.objects.filter(employee_id=user_id, time__gte=time_threshold) \
                                        .values('emotion_data') \
                                        .annotate(count=Count('emotion_data')) \
                                        .order_by('emotion_data') \
                                        .values_list('emotion_data', 'count')
        else:
            emotion_counts_list = Employee_Emotion.objects.filter(employee_id=user_id) \
                                        .values('emotion_data') \
                                        .annotate(count=Count('emotion_data')) \
                                        .order_by('emotion_data') \
                                        .values_list('emotion_data', 'count')
        ...
        for hour in range(start_hour, end_hour + 1):
            ...
            hourly_emotion = Employee_Emotion.objects.filter(employee_id=user_id, time__gte=start_time, time__lt=end_time) \
                                  .values('emotion_data') \
                                  .annotate(count=Count('emotion_data')) \
                                  .order_by('-count') \
                                  .first()
