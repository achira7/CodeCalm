...
import Img from "../../assets/StressForm/Forms.png";

const StressDetectionForm = () => {
  const [questions, setQuestions] = useState([
    {
      id: 6,
      question: "How often you were stressed during the week?",
      affect: "N",
      type: "T",
      timestamp: "2024-06-16T10:24:06.102068Z",
    },
  ]);
  ...
  const fetchQuestions = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/stress_questions/",
        {
          withCredentials: true,
        }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/api/stress_form/",
        submissionData,
        {
          withCredentials: true,
        }
    ...
    >
      ...
      <div className="flex flex-col ">
        <div className="flex justify-center w-full">
          <div className="flex justify-center">
            <img src={Img} className="w-[300px]" alt="form Image" />
          </div>
        </div>
