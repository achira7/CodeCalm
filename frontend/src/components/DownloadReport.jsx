...
export const downloadPDF = async ({componenetName, team, name, userRole, orientation}) => {
    ...
    html2canvas(input, { scale: 1 })
      .then((canvas) => {
        ...
        if (team) {
          ...
          pdf.text(`Employee ${name} ${componenetName} Report`, 10, 10);
        }
  
        pdf.setTextColor(0, 0, 0);
        pdf.text(`Generated on: ${timestamp}`, 10, 15);
        pdf.text(`Generated By: ${userRole} - ${name}`, 10, 20);
  
        if (team) {
          pdf.save(`team_${team}_${componenetName} report_${timestamp}.pdf`);
        } else {
          pdf.save(`employee_${name}_${componenetName} report_${timestamp}.pdf`);
        }
      })
      .catch((error) => {
